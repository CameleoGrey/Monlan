...
class DQNAgent:
    ...
    def buildSimpleConvModel(self):
        ...
        headModel = Conv2D(filters=64, kernel_size=4, strides=(3, 3), padding="same", activation="tanh")(headModel)
        # headModel = Dropout(0.1)(headModel)
        headModel = Conv2D(filters=64, kernel_size=4, strides=(2, 2), padding="same", activation="tanh")(headModel)
        # headModel = Dropout(0.1)(headModel)
        headModel = Conv2D(filters=128, kernel_size=4, strides=(2, 2), padding="same", activation="tanh")(headModel)
        #headModel = Dropout(0.1)(headModel)
        ...
        """mainInputShape = (self.state_size[0], self.state_size[1], 1)
        mainInput = Input(shape=mainInputShape)
        headModel = Conv2D(filters=32, kernel_size=3, padding="same", activation="relu",
                           kernel_initializer="uniform")(mainInput)
        headModel = MaxPool2D(pool_size=(1, 2))(headModel)
        headModel = Conv2D(filters=64, kernel_size=3, padding="same", activation="relu",
                           kernel_initializer="uniform")(headModel)
        headModel = MaxPool2D(pool_size=(1, 2))(headModel)
        headModel = Conv2D(filters=128, kernel_size=3, padding="same", activation="relu",
                           kernel_initializer="uniform")(headModel)
        headModel = MaxPool2D(pool_size=(1, 2))(headModel)
        headModel = Conv2D(filters=64, kernel_size=3, padding="same", activation="relu",
                           kernel_initializer="uniform")(headModel)
        ...
        headModel = Flatten()(headModel)
        headModel = Dense(32, activation="relu", kernel_initializer="uniform")(headModel)
        headModel = Dense(64, activation="relu", kernel_initializer="uniform")(headModel)
        headModel = Dense(128, activation="relu", kernel_initializer="uniform")(headModel)
        headModel = Dense(64, activation="relu", kernel_initializer="uniform")(headModel)
        headModel = Dense(32, activation="relu", kernel_initializer="uniform")(headModel)
        #headModel = CuDNNLSTM(512, return_sequences=False)(headModel)
        headModel = Dense(self.action_size, activation="tanh", kernel_initializer='uniform')(headModel)
        model = Model(inputs=mainInput, outputs=headModel)"""
        ...
        # headModelInput = Input(shape=(self.state_size[1], self.state_size[2], 1))(headModelInput)
        # headModelInput._keras_shape = (None, headModelInput.keras_shape[2], headModelInput.keras_shape[3], headModelInput.keras_shape[4] )
        headModel = Conv2D(filters=32, kernel_size=16, strides=(8, 8), padding="same", activation='linear', kernel_initializer='he_normal')(mainInput)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("elu")(headModel)
        #headModel = Dropout(0.5)(headModel)
        headModel = Conv2D(filters=64, kernel_size=8, strides=(4, 4), padding="same", activation='linear', kernel_initializer='he_normal')(headModel)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("elu")(headModel)
        #headModel = Dropout(0.5)(headModel)
        headModel = Conv2D(filters=128, kernel_size=4, strides=(3, 3), padding="same",activation='linear', kernel_initializer='he_normal')(headModel)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("elu")(headModel)
        headModel = Dropout(0.5)(headModel)
        headModel = Conv2D(filters=256, kernel_size=3, strides=(2, 2), padding="same",activation='linear', kernel_initializer='he_normal')(headModel)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("elu")(headModel)
        headModel = Dropout(0.5)(headModel)
        headModel = Conv2D(filters=512, kernel_size=2, strides=(1, 1), padding="same", activation='linear', kernel_initializer='he_normal')(headModel)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("elu")(headModel)
        ###############################
        #headModel = Reshape((headModel._keras_shape[1] * headModel._keras_shape[2], headModel._keras_shape[3]))(headModel)
        ...
        ###############################
        headModel = Flatten()(headModel)
        headModel = Dense(self.action_size, activation="linear", kernel_initializer="glorot_uniform")(headModel)
        headModel = BatchNormalization()(headModel)
        headModel = Activation("tanh")(headModel)
        #headModel = Dense(self.action_size, activation="linear", kernel_initializer="uniform")(headModel)
        model = Model(inputs=mainInput, outputs=headModel)"""
