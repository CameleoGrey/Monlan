...
class DDQNAgent:
    ...
    def buildSimpleConvModel(self):
        ...
        headModel = Conv2D(filters=32, kernel_size=5, padding="same", activation="tanh",
                           kernel_initializer="glorot_uniform")(mainInput)
        # headModel = MaxPool2D(pool_size=(2, 2))(headModel)
        headModel = Conv2D(filters=32, kernel_size=5, padding="same", activation="tanh",
                           kernel_initializer="glorot_uniform")(headModel)
        # headModel = MaxPool2D(pool_size=(2, 2))(headModel)
        ...
        headModel = Flatten()(headModel)
        # headModel = CuDNNLSTM(512, return_sequences=False)(headModel)
        bodyModel = Dense(self.action_size, activation="tanh", kernel_initializer="glorot_uniform")(headModel)
        model = Model(inputs=mainInput, outputs=bodyModel)
    ...
    def train_model(self):
        ...
        update_input = np.zeros((batch_size, self.state_size[0]))
        update_target = np.zeros((batch_size, self.state_size[0]))
        action, reward, done = [], [], []
