...
class FeatGen_CDV():
    ...
    """def transform(self, df, period=14, verbose=True):
        ...
        selling_vol = []
        if verbose:
            cycle_range = tqdm(range(open.shape[0]), desc="delta", colour="g")
        else:
            cycle_range = range(open.shape[0])
        for i in cycle_range:
            bv, sv = self.get_delta_( open[i], high[i], low[i], close[i], volume[i] )
            buying_vol.append( bv )
            selling_vol.append( sv )

        nan_vals = []
        for i in range( period - 1 ):
            nan_vals.append( 0 )
        cum_buying_volume = self.ema_( buying_vol, period )
        cum_selling_volume = self.ema_( selling_vol, period )

        cum_buying_volume = np.array( cum_buying_volume )
        cum_selling_volume = np.array(cum_selling_volume)
        cum_delta = cum_buying_volume - cum_selling_volume
    ...
    def transform(self, df, period=14, verbose=False):
