...
class PlotRender():
    ...
    def plot_candles_(self, df):

        df = df.copy()
        df["index"] = df.index.array
        df["datetime"] = pd.to_datetime(df["datetime"])
        df.set_index(keys=["datetime"], inplace=True)

        ohlc = df.loc[:, ["open", "high", "low", "close"]]

        fig, ax = mf.plot(ohlc, type="candle", returnfig=True, tight_layout=True, ylabel=".")
        #ax[1].figure.set_facecolor('white')
        plt.show()
