...
from legacy.classes.monlan.agents.RiskManager import RiskManager
import time
...
class CompositeEnv():
    ...
    def get_std_stop(self, dealType, lastObs, nPoints=32):

        df = self.historyPrice[:lastObs[0]].tail(nPoints)
        open = df["open"].values
        high = df["high"].values
        low = df["low"].values
        ...
        for i in range( len(open) ):
            if open[i] < close[i]:
                deltas.append(high[i] - close[i])
                deltas.append(open[i] - low[i])
            if close[i] < open[i]:
                deltas.append(high[i] - open[i])
                deltas.append(close[i] - low[i])
        mean_delta = np.mean( deltas )
        stop_val = 3*mean_delta # + 0.3*mean_delta
        stop_val = int(self.lotCoef * stop_val)

        return stop_val

    def get_std_take(self, dealType, lastObs, nPoints=32):
        ...
        for i in range( len(open) ):
            ...
            if close[i] < open[i]:
                deltas.append( close[i] - low[i] )
        mean_delta = np.mean( deltas )
        take_val = 12*mean_delta # + 0.3*mean_delta
        take_val = int(self.lotCoef * take_val)
    ...
    def step(self, action):
        ...
        if self.mode == "opener":
            if action == "buy":
                ...
                if self.stopType == "adaptive" or self.takeType == "adaptive":
                    ...
                    if limitsDict["stop"] < 2 * self.openPoint[1]["spread"] + 1:
                        limitsDict["stop"] = 2 * self.openPoint[1]["spread"] + 1
                    if limitsDict["take"] < 2 * self.openPoint[1]["spread"] + 1:
                        limitsDict["take"] = 2 * self.openPoint[1]["spread"] + 1
                    if self.stopType == "adaptive":
                        self.stoplossPuncts = limitsDict["stop"]
                        if self.stoplossPuncts > self.maxLoss:
                            self.stoplossPuncts = self.maxLoss
                    if self.takeType == "adaptive":
                        self.takeprofitPuncts = limitsDict["take"]
                        if self.takeprofitPuncts > self.maxTake:
                            self.takeprofitPuncts = self.maxTake

                if self.stopType == "std":
                    self.stoplossPuncts = self.get_std_stop("buy", nextHistoryRow, nPoints=64)
                    min_pos_limit = 2 * self.openPoint[1]["spread"] + 1
                    if self.stoplossPuncts < min_pos_limit:
                        self.stoplossPuncts = min_pos_limit
                    if self.takeprofitPuncts < min_pos_limit:
                        self.takeprofitPuncts = min_pos_limit
                if self.takeType == "std":
                    self.takeprofitPuncts = self.get_std_take("buy", nextHistoryRow, nPoints=64)
                    min_pos_limit = 2 * self.openPoint[1]["spread"] + 1
                    if self.stoplossPuncts < min_pos_limit:
                        self.stoplossPuncts = min_pos_limit
                    if self.takeprofitPuncts < min_pos_limit:
                ...
                if self.stopType == "adaptive" or self.takeType == "adaptive":
                    ...
                    if self.takeType == "adaptive":
                        ...
                        if self.takeprofitPuncts > self.maxTake:
                            self.takeprofitPuncts = self.maxTake
                if self.stopType == "min_possible":
                    self.stoplossPuncts = 2 * self.openPoint[1]["spread"] + 1
                if self.takeType == "min_possible":
                    self.takeprofitPuncts = 1 * self.openPoint[1]["spread"] + 1
                if self.stopType == "std":
                    self.stoplossPuncts = self.get_std_stop("sell", nextHistoryRow, nPoints=32)
                    min_pos_limit = 2 * self.openPoint[1]["spread"] + 1
                    if self.stoplossPuncts < min_pos_limit:
                        self.stoplossPuncts = min_pos_limit
                    if self.takeprofitPuncts < min_pos_limit:
                        self.takeprofitPuncts = min_pos_limit
                if self.takeType == "std":
                    self.takeprofitPuncts = self.get_std_take("sell", nextHistoryRow, nPoints=32)
                    min_pos_limit = 2 * self.openPoint[1]["spread"] + 1
                    if self.stoplossPuncts < min_pos_limit:
                        self.stoplossPuncts = min_pos_limit
                    if self.takeprofitPuncts < min_pos_limit:
                ...
                self.sumLoss += -0.1 * (abs(nextHistoryRow[1]["close"] - nextHistoryRow[1]["open"]) + self.spreadCoef * nextHistoryRow[1]["spread"]) * self.lotCoef * self.lotSize
        ...
        if self.mode in ["buyer", "seller"]:
            if action == "hold":
                obs = self.featureGenerators[self.mode].getFeatByDatetime(nextHistoryRow[0], self.historyPrice)
                if self.mode == "buyer":
                    reward = (nextHoldRow[1]["close"] - (self.openPoint[1]["open"] + self.spreadCoef * self.openPoint[1]["spread"])) * self.lotCoef * self.lotSize
                elif self.mode == "seller":
                    #reward = (self.openPoint[1]["open"] - (nextHoldRow[1]["close"] + 0.5 * nextHoldRow[1]["close"])) * self.lotCoef * self.lotSize
                    reward = (self.openPoint[1]["open"] - (nextHoldRow[1]["close"] + self.spreadCoef * nextHoldRow[1]["spread"])) * self.lotCoef * self.lotSize
                if self.renderFlag == True:
                    self.render(self.iStep, nextHistoryRow[1]["open"], nextHistoryRow[1]["close"], action)
                ...
                rewardDict = {}
                if self.mode == "buyer":
                        rewardDict[0] = (nextHoldRow[1]["close"] - (self.openPoint[1]["open"] + self.spreadCoef * self.openPoint[1]["spread"])) * self.lotCoef * self.lotSize
                        rewardDict[1] = (nextHistoryRow[1]["close"] - (self.openPoint[1]["open"] + self.spreadCoef * self.openPoint[1]["spread"])) * self.lotCoef * self.lotSize
                elif self.mode == "seller":
                        rewardDict[0] = (self.openPoint[1]["open"] - (nextHoldRow[1]["close"] + self.spreadCoef * nextHoldRow[1]["spread"])) * self.lotCoef * self.lotSize
                        rewardDict[1] = (self.openPoint[1]["open"] - (nextHistoryRow[1]["close"] + self.spreadCoef * nextHistoryRow[1]["spread"])) * self.lotCoef * self.lotSize

                #################################
                info["takeprofit"] = self.isTriggeredTakeprofit(nextHistoryRow, self.mode)
                info["stoploss"] = self.isTriggeredStoploss(nextHistoryRow, self.mode)
                ...
                return obs, reward, self.done, info
            if action == "buy":
                reward = (nextHistoryRow[1]["close"] - (self.openPoint[1]["open"] + self.spreadCoef * self.openPoint[1]["spread"])) * self.lotCoef * self.lotSize
            elif action == "sell":
                reward = (self.openPoint[1]["open"] - (nextHistoryRow[1]["close"] + self.spreadCoef * nextHistoryRow[1]["spread"])) * self.lotCoef * self.lotSize
            if self.renderFlag == True:
                self.render(self.iStep, nextHistoryRow[1]["open"], nextHistoryRow[1]["close"], action)
            ...
            ######################################
            info["takeprofit"] = self.isTriggeredTakeprofit(nextHistoryRow, saveMode)
            info["stoploss"] = self.isTriggeredStoploss(nextHistoryRow, saveMode)
            info["limitOrder"] = self.checkLimitTrigger(info["takeprofit"], info["stoploss"])
            if info["limitOrder"]:
                rewardDict = self.getLimitReward(info["takeprofit"], info["stoploss"])
                self.mode = "opener"
    ...
    def isTriggeredStoploss(self, nextHistoryRow, checkMode):
        # stoploss
        info = {}
        info["stoploss"] = False
        if checkMode == "buyer":
            stoplossPrice = ((self.openPoint[1]["open"] - self.spreadCoef * (self.stoplossPuncts - self.openPoint[1]["spread"]))) * self.lotCoef * self.lotSize
            nextExtremum = nextHistoryRow[1]["low"] * self.lotCoef * self.lotSize
            if nextExtremum - stoplossPrice <= 0:
                info["stoploss"] = True
        elif checkMode == "seller":
            stoplossPrice = ((self.openPoint[1]["open"] + self.spreadCoef * (self.stoplossPuncts - self.openPoint[1]["spread"]))) * self.lotCoef * self.lotSize
    ...
    def isTriggeredTakeprofit(self, nextHistoryRow, checkMode):
        # stoploss
        info = {}
        info["takeprofit"] = False
        if checkMode == "buyer":
            takeprofitPrice = ((self.openPoint[1]["open"] + self.spreadCoef * (self.takeprofitPuncts + self.openPoint[1]["spread"]))) * self.lotCoef * self.lotSize
            nextExtremum = nextHistoryRow[1]["high"] * self.lotCoef * self.lotSize
