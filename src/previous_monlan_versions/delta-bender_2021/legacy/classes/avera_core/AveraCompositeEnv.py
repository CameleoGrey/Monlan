...
class AveraCompositeEnv():
    ...
    def __init__(self, historyPrice, openerFeatureGen, buyerFeatureGen, sellerFeatureGen, startDeposit = 300,
                 ...
                 renderFlag=True, renderDir=None, renderName=None ):
        ...
        if self.mode == "opener":
            ...
            if action == "buy":
                ...
                if self.stopType == "adaptive" or self.takeType == "adaptive":
                    limitsDict = self.featureGenerators[self.mode].getAdaptiveLimits(nextDate,
                                                                                     self.historyPrice,
                                                                                     self.openPoint[1]["open"],
                                                                                     self.spreadCoef,
                                                                                     stopPos=self.stopPos,
                                                                                     takePos=self.takePos)
                ...
                if self.stopType == "adaptive" or self.takeType == "adaptive":
                    ...
                    if self.takeprofitPuncts > self.maxTake:
                        self.takeprofitPuncts = self.maxTake
                self.mode = "seller"
                obs = self.featureGenerators[self.mode].getFeatByDatetime(nextDate, self.historyPrice, self.openPoint[1]["open"])
            elif action == "hold":
                #self.sumLoss += -0.2 * (0.5 * abs(nextHistoryRow[1]["close"] - nextHistoryRow[1]["open"]) + self.spreadCoef * self.spread) * self.lotCoef * self.lotSize
                # self.sumLoss += -np.pi * np.sqrt(self.spread) * self.spreadCoef
                #self.sumLoss += -np.pi * self.spreadCoef
                #reward = self.sumLoss
        ...
        if self.mode in ["buyer", "seller"]:
            if action == "hold":
                ...
                if info["limitOrder"]:
                    ...
                    #reward = self.getLimitReward(info["takeprofit"], info["stoploss"])[0]
                    self.mode = "opener"
                    nextOpenerObs = self.featureGenerators[self.mode].getFeatByDatetime(nextDate, self.historyPrice)
                    #if len(nextOpenerObs.shape) == 1:
                    #    nextOpenerObs = nextOpenerObs.reshape((1, -1))
                    info["nextOpenerState"] = np.reshape(nextOpenerObs, (1,) + nextOpenerObs.shape )
                    #info["nextOpenerState"] = np.reshape(info["nextOpenerState"], info["nextOpenerState"].shape + (1,))
                    self.deposit += rewardDict[0]
                    print("Limit order triggered. Reward = {}".format(rewardDict[0]))
                    #self.deposit += reward
                    #print("Limit order triggered. Reward = {}".format(reward))
                ######################################
            ...
            nextCloserObs = self.featureGenerators[self.mode].getFeatByDatetime(nextDate, self.historyPrice, self.openPoint[1]["open"])
            self.mode = "opener"
            nextOpenerObs = self.featureGenerators[self.mode].getFeatByDatetime(nextDate, self.historyPrice)
            ...
            if info["limitOrder"]:
                rewardDict = self.getLimitReward(info["takeprofit"], info["stoploss"])
                #reward = self.getLimitReward(info["takeprofit"], info["stoploss"])[1]
                self.mode = "opener"
                info["nextOpenerState"] = np.reshape( nextOpenerObs, (1,) + nextOpenerObs.shape )
                #info["nextOpenerState"] = np.reshape(info["nextOpenerState"], info["nextOpenerState"].shape + (1,))
                print("Limit order triggered. Reward = {}".format(rewardDict[0]))
            ...
            nextOpenerObs = nextOpenerObs.reshape((1,) + nextOpenerObs.shape)
            #nextOpenerObs = nextOpenerObs.reshape(nextOpenerObs.shape + (1,))

            nextCloserObs = nextCloserObs.reshape((1,) + nextCloserObs.shape)
            #nextCloserObs = nextCloserObs.reshape(nextCloserObs.shape + (1,))
