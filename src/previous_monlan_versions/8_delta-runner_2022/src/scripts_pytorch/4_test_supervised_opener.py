...
priceDiffGenerator = FeatGen_ScaledWindow(hkFeatList, nPoints=48, flatStack=False)
...
#testDf = mod_df.tail(230000).head(100000)
...
composite_agent = load( os.path.join("../../models/", "composite_agent_{}.pkl".format(13)))
supervised_opener = SupervisedOpener( "opener", testEnv.observation_space["opener"], testEnv.action_space["opener"].n )
so_model = load(os.path.join( "../../models", "supervised_opener_network.pkl" ))
...
score, dealsStatistics = composite_agent.use_agent(testEnv, render_deals=True)
