...
openerScaleGen = ScalerGenerator(featureList=obsFeatList, nPoints=50, flatStack=True,  fitOnStep=False)
openerDiffGen = DiffGenerator(featureList=obsFeatList,  nDiffs=1, nPoints=50, flatStack=True, fitOnStep=False)
openerFeatureFactory = CompositeGenerator(genList=[openerScaleGen, openerDiffGen])
buyerScaleGen = ScalerGenerator(featureList=obsFeatList, nPoints=50, flatStack=True, fitOnStep=False)
buyerDiffGen = DiffGenerator(featureList=obsFeatList,  nDiffs=1, nPoints=50, flatStack=True, fitOnStep=False)
buyerFeatureFactory = CompositeGenerator(genList=[buyerScaleGen, buyerDiffGen])
sellerScaleGen = ScalerGenerator(featureList=obsFeatList, nPoints=50, flatStack=True, fitOnStep=False)
sellerDiffGen = DiffGenerator(featureList=obsFeatList,  nDiffs=1, nPoints=50, flatStack=True, fitOnStep=False)
sellerFeatureFactory = CompositeGenerator(genList=[sellerScaleGen, sellerDiffGen], flatStack=True)
...
trainEnv = CompositeEnv(trainDf, openerFeatureFactory, buyerFeatureFactory, sellerFeatureFactory,
...
agent.fit_agent(env=trainEnv, nEpisodes=10000, plotScores=True, saveFreq=10) #TODO: add save best only
