...
priceDiffGenerator = MultiScalerDiffGenerator(featureList=hkFeatList, nDiffs=1, nPoints = 512, flatStack = False, fitOnStep = False)
...
buyerPriceDiffGenerator = RelativeDiffGenerator(featureList=hkFeatList).loadGenerator("./RDiffGen.pkl")
sellerPriceDiffGenerator = RelativeDiffGenerator(featureList=hkFeatList).loadGenerator("./RDiffGen.pkl")
...
trainEnv = CompositeEnv(trainDf, openerPriceDiffGenerator, buyerPriceDiffGenerator, sellerPriceDiffGenerator,
                        ...
                        stoplossPuncts=100000, takeprofitPuncts=100000, renderFlag=True, renderDir="./", renderName="train_plot")
...
backTestEnv = CompositeEnv(backTestDf, openerPriceDiffGenerator, buyerPriceDiffGenerator, sellerPriceDiffGenerator,
                        startDeposit=300, lotSize=0.1, lotCoef=100000, spread=18, spreadCoef=0.00001,
                           stoplossPuncts=200, takeprofitPuncts=400, renderFlag=True,
...
openerAgent = DQNAgent(trainEnv.observation_space["opener"], trainEnv.action_space["opener"].n,
                           memorySize=2500, batch_size=500, train_start=2400, epsilon_min=0.1, epsilon=1,
