...
buyer_agent = load(os.path.join("../../../../../models", "best_val_distilled_tcn_mirror_closer.pkl"))
buyer_agent.name = "buyer"
...
buyer_feat_generator = FeatGen_HistoryEmbedder( base_feat_generator, buyer_agent.model )
...
buyer_feat_generator.fit( history_price_array, price_feature_names_dict, start_point, batch_size=64 )
for i in tqdm(range(start_point, len(history_df) - 2), desc="Collecting buyer statistics"):
    ...
    open_point_embedding = buyer_feat_generator.cached_embeddings[i]
    raw_open_features.append(open_point_raw_features)
    open_embeddings.append(open_point_embedding)

    open_embedding = buyer_feat_generator.get_features(history_price_array, price_feature_names_dict, i)
    action = buyer_agent.get_action_head_only( open_embedding )
    ...
    while state_descriptor.action != 1:
        ...
        next_obs = buyer_feat_generator.get_features(history_price_array, price_feature_names_dict, j)
        action = buyer_agent.get_action_head_only( next_obs )
...
buyer_samples = { "id": steps_i,
                  ...
                  "low": low_price,
                  "close": close_price }
save(buyer_samples, os.path.join("../../../../../data/interim", "distilled_buyer_samples.pkl"))
print("done")