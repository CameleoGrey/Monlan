...
seller_feat_generator = stub_generator
seller_base_feat_generator = stub_generator

history_df = mod_df.tail(340000).head(300000)
composite_env = CompositeEnv(history_df, opener_base_feat_generator, seller_feat_generator, seller_base_feat_generator,
...
for i in tqdm(range(start_point, len(history_df) - 2), desc="Collecting seller statistics"):
    ...
    state_descriptor = seller_env.step(history_price_array, price_feature_names_dict, i, 0)
    ...
    for estimate_i in range( n_estimates ):
        ...
        while state_descriptor.action != 1:
            ...
            action = seller_agent.get_action_head_only( current_hold_proba, current_close_proba )
