...
class W2VDiffGenerator():
    ...
    def createW2VModelAndDict(self, textPrices):
        ...
        w2vModel = word2vec.Word2Vec(docsList, size=self.w2vSize, window=self.window,
                                     workers=10, iter=self.iter, min_count=self.min_count, sample=self.sample, sg=self.sg)
    ...
    def checkReconstructionQuality(self, df):
        ...
        for feat in self.featureList:
            tmp = []
            for i in tqdm(range(len(vectorPrices[feat])), desc="Reconstructing raw features from w2v"):
