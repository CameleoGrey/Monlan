...
class SellerEnv(EnvInterface):
    def __init__(self, feature_generator, spread_coef, lot_coef, lot_size, turn_off_spread):
        ...
        class SellerActionSpace():
            def __init__(self):
                ...
                self.n = 2
                pass
        self.action_space = SellerActionSpace()
    ...
    def step(self, history_price_array, price_feature_names_dict, history_step_id, action_id):
        ...
        if self.open_point > history_step_id:
            raise ValueError("Wrong order of open and close points in seller env")
        ...
        reward_dict[0] = (open_point_open_price - (two_hop_open_price + self.spread_coef * two_hop_spread)) * self.lot_coef * self.lot_size
        reward_dict[1] = (open_point_open_price - (one_hop_open_price + self.spread_coef * one_hop_spread)) * self.lot_coef * self.lot_size
        #reward_dict[0] = (current_open_price - (two_hop_open_price + self.spread_coef * two_hop_spread)) * self.lot_coef * self.lot_size
        #reward_dict[1] = (current_open_price - (one_hop_open_price + self.spread_coef * one_hop_spread)) * self.lot_coef * self.lot_size

        action = self.action_space.actionsDict[action_id]
        if action == "sell":
            if self.open_point is None:
                raise ValueError("Seller open_point is None")
    ...
    def is_triggered_stoploss(self, history_price_array, price_feature_names_dict, history_step_id):
        ...
        stoploss_price = ((open_point_open_price + self.spread_coef * (self.stoploss_puncts - open_point_spread))) * self.lot_coef * self.lot_size
        next_extremum = current_high_price * self.lot_coef * self.lot_size
        if stoploss_price - next_extremum <= 0:
            info["stoploss"] = True
    ...
    def is_triggered_takeprofit(self, history_price_array, price_feature_names_dict, history_step_id):
        ...
        if next_extremum - takeprofit_price <= 0:
