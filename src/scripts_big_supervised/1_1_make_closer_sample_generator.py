...
################################################################
# Making on-fly sample generator for 1 symbol only (for test purpose).
...
sample_generator.fit( history_df, composite_env,
                      norm_ema=20, smooth_ema=20,
                      start_hold_proba=0.2, end_hold_proba=0.95, n_estimates=100,
                      show_plots=False )

save( sample_generator, os.path.join( "../../data/interim/sample_generator.pkl" ) )