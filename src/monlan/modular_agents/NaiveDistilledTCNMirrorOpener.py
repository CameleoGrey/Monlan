...
class NaiveDistilledTCNMirrorOpener:
    ...
    def fit(self, buyer_samples, seller_samples,
            epochs = 202, warm_up_epochs = 2, batch_size = 16, learning_rate = 0.0001,
    ...
    def get_action(self, state):
        ...
        q_value = q_value[0]
        if self.name == "opener":
            q_value = [q_value[0], 0.16, q_value[1]]
        else:
            raise ValueError("Name of mirror agent must be specified (opener) before using")
