...
# sell deal in each time step of training data.
################################################################
...
opener_feat_gen = price_diff_generator
buyer_feat_gen = price_diff_generator
seller_feat_gen = price_diff_generator

reward_transformer = load( os.path.join("../../models/reward_transformer.pkl") )

test_df = mod_df.tail(340000).head(300000)
composite_env = CompositeEnv(test_df, opener_feat_gen, buyer_feat_gen, seller_feat_gen,
...
for i in tqdm(range(start_point, len(test_df) - 2), desc="Collecting seller statistics"):
